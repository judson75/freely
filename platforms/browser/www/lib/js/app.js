...
//	document.addEventListener("active", onDeviceActive, false);
		...
		*/
		...
		request.done(function(data) { 
			...
			if(obj.resp === 'success') {
				...
				profile_nav += '<a href="profile.html?user_slug=' + obj.user.username + '" data-transition="slide">My Profile</a>';
				...
				if(obj.user.friends !== '') {
					...
					$.each( obj.user.friends, function( key, friend ) {
						...
						profile_nav += '<a href="profile.html?user_slug=' + friend.username + '" data-transition="slide">';
						...
						profile_nav += '<a href="profile.html?user_slug=' + friend.username + '" data-transition="slide">' + friend.name + '</a>';
						profile_nav += '</div>';
	...
	$('#home, index.html').on( "pagebeforeshow", function( event ) { 
		//console.log("HOME: " + common.serviceURL + " - EVENT: " + event);
		var user = common.storage.getItem("app_user");
		//Ajax to get feed
		//$('#home-post').html('');
		$.mobile.loading('show');
		...
		request.done(function(data) { 
			...
			if(obj.code == 1) {
				...
				var html = '<div id="first_post_marker"></div>';
				$.each(obj.posts, function( index, post ){
					html += postDisplay(post, user);
				});
				$('#home-post').html(html, user);
				$.mobile.loading('hide');
			}
		});
		request.fail(function(jqXHR, textStatus, thrownError) {			
	...
	$('#profile-page, #profile').on( "pagebeforeshow", function( event ) {
		var user = common.storage.getItem("app_user"); 
	...
	function postDisplay(post, user) {
		...
		if(post.news_url) {
			...
			html += '<a href="#profile-page?user_id=' + post.user_id + '" data-transition="slide" data-id="' + post.user_id + '"><img src="' + post.poster_avatar + '" class="media-object"></a>';
			html += '</div>';
			html += '<div class="media-body"><a href="#profile-page?user_id=' + post.user_id + '" data-transition="slide" data-id="' + post.user_id + '"> ' + post.poster_name + '</a>';
			if(post.reply_to) {
				console.log(post.reply_to);
				//html += post.reply_to;
				html += ' <i class="fa fa-caret-right" aria-hidden="true"></i>  <a href="#profile-page?user_id=' + post.reply_to.user_id + '" data-transition="slide">' + post.reply_to.name + '</a> ';
			}
			...
			if(post.media !== '' && post.media !== null && post.media !== undefined) {
				...
				if(post.media.length > 3) {
					...
					html += '<div class="img-blk-one-thirds" id="img-' + post.media[1].id + '" style="margin-bottom: 5px;"><a href="' + common.siteURL + '/post/' + post.id + '#img-' + post.media[1].id +'"><img src="' + common.siteURL + '/lib/php/timthumb+php?src=' + common.siteURL + '/lib/images/status_images/' + post.media[1].file_name +  '&h=148&w=210&zc=1" class="img-responsive"></a></div>';
					html += '<div class="img-blk-one-thirds" id="img-' + post.media[2].id + '"><a href="' + common.siteURL + '/post/' + post.id + '#img-' + post.media[2].id +'"><img src="' + common.siteURL + '/lib/php/timthumb+php?src=' + common.siteURL + '/lib/images/status_images/' + post.media[2].file_name +  '&h=148&w=210&zc=1" class="img-responsive"></a></div>';
					html += '<div class="clr"></div>';
				}
					...
					html += '<div class="img-blk-half" id="img-' + post.media[1].id + '" style="margin-bottom: 5px;"><a href="' + common.siteURL + '/post/' + post.id + '#img-' + post.media[1].id +'"><img src="' + common.siteURL + '/lib/php/timthumb+php?src=' + common.siteURL + '/lib/images/status_images/' + post.media[1].file_name +  '&h=148&w=210&zc=1" class="img-responsive"></a></div>';
					html += '<div class="clr"></div>';
				}
				else {
					//if($ext == 'jpg' || $ext == 'gif') {
...
//console.log(show_likes);
				...
				if(post.likes.length === 1) {
					html += '<a href="#profile-page?user_id=' + post.likes[0].user_id + '" data-transition="slide" data-user="' + post.likes[0].username + '">' + show_likes[0].name + '</a>';
				}
				else if(post.likes.length === 2) {
					html += '<a href="#profile-page?user_id=' + post.likes[0].user_id + '" data-transition="slide" data-user="' + post.likes[0].username + '">' + show_likes[0].name + '</a> and <a href="#profile-page?user_id=' + post.likes[1].user_id + '" data-transition="slide" data-user="' + post.likes[1].username + '">'  + show_likes[1].name + '</a>';
				}
				else {
					var liked_remainder = post.likes.length - 2;
					html += '<a href="#profile-page?user_id=' + post.likes[0].user_id + '" data-transition="slide" data-user="' + post.likes[0].username + '">' + show_likes[0].name + '</a>, <a href="#profile-page?user_id=' + post.likes[1].user_id + '" data-transition="slide" data-user="' + post.likes[1].username + '">'  + show_likes[1].name + '</a> and ' + liked_remainder + ' others';
				}
			...
			html += '</span>';
			html += '<span class="media-action-btn commentPostBtn" data-id="' + post.id + '" data-user="' + user + '" id="comment-btn-' + post.id + '"><a href="#post-page" data-transition="slide" data-id="' + post.id + '" data-user="' + user + '"><i class="fa fa-comments-o" aria-hidden="true"></i> Comment';
		...
		/*
		html += '<ul class="comments" id="comments-78">

			<li class="media post-container-91" id="panel-body-91">
			<div class="media-left">
				<a href="http://cbhllc.net/profile/gary"><img src="http://cbhllc.net/lib/php/timthumb.php?src=http://cbhllc.net/lib/images/user_images/20171125_174825.jpg&amp;h=40&amp;w=40&amp;zc=1" class="media-object"></a></div>
			<div class="media-body"><a href="http://cbhllc.net/profile/gary" class="comment-author">Gary Cooper</a> 
				<span class="comment-date">Just Now</span>
							</div>
							<div class="clr"></div>
							<div class="comments-reply" id="post-container-T">Told you guys </div></li></ul>
		*/
		html += '</div>';
		html += '</div>';
		return html;
